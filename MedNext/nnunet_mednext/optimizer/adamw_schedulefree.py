...
import math
# 定义全局变量 is_training，表示当前是否处于训练模式
is_training = True  # 默认为训练模式，可以根据需要更改
...
class AdamWScheduleFree(torch.optim.Optimizer):
    ...
    def eval(self):
        global is_training
        is_training = False  # 设置全局变量为 False，表示处于测试模式
    ...
    def train(self):
        global is_training
        is_training = True  # 设置全局变量为 True，表示处于训练模式
        for group in self.param_groups:
            train_mode = group['train_mode']
            beta1, _ = group['betas']
            if not train_mode:
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        """执行一个优化步骤"""
        if is_training:
            self.train()  # 如果是训练模式，则调用 train 方法
        else:
            self.eval()  # 如果是测试模式，则调用 eval 方法
